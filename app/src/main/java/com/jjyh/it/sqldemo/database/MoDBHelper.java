...
import com.jjyh.it.sqldemo.bean.ChatMessage;
...
public class MoDBHelper extends DataBaseHelper {
    private final static String tag = "modb";
    private static MoDBHelper mHelper;

    private MoDBHelper(Context context){
        super(context);
    }



    public static MoDBHelper getInstance(Context context){
        if (mHelper==null) {
            synchronized (DataBaseHelper.class){
                if (mHelper==null){
                    mHelper = new MoDBHelper(context);
                    if (mHelper.getDB() == null) {
                        mHelper.open();
                    } else if(!mHelper.getDB().isOpen()) {
                        mHelper.open();
                    }
                }
            }
    ...
    public void deleteChatRecord() {
        String sql = "delete from " + ChatsRecord.TABLE_CHATSRECORD;
        this.mDb.execSQL(sql);
    }

    public void insertChatRecord(String info,int type) {
        ContentValues values = new ContentValues();
        values.put(ChatsRecord.CONTENTS, info);
        values.put(ChatsRecord.TYPE, type);
        long rs = insert(ChatsRecord.TABLE_CHATSRECORD,values);
        Log.i(tag,"insertChatRecord:"+rs);
    }

    public void updateChatRecord(List<ChatMessage> list) {
        String sql = "select * from "+ChatsRecord.TABLE_CHATSRECORD;
        Cursor cursor = this.mDb.rawQuery(sql,null);
        while (cursor!=null && cursor.moveToNext()) {
            String body = cursor.getString(cursor.getColumnIndex(ChatsRecord.CONTENTS));
            int type = cursor.getInt(cursor.getColumnIndex(ChatsRecord.TYPE));
            int d = 0;
            if (type == 0) {
                d = ChatMessage.SEND;
            }
            if (type == 1) {
                d = ChatMessage.RECEIVE;
            }
            list.add(new ChatMessage(d,body,type));
        }
